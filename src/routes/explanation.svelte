...
<section>
    ...
    <ul class="content">
        <li>The app uses the <a href="https://kit.svelte.dev/" target="_blank">Sveltekit</a> framework which provides both the frontend (user-interface) and backend for the app.</li>
...
<section>
    ...
    <div class="content">
        ...
        <ul>
            ...
            <li>If there is an error in the update, or if the simulate error option was selected, the endpoint sends an update to the telegram bot api</li>
...
<section>
    ...
    <div class="content">
        ...
        <p>Relational data refers to data that is highly organizable through the use of multiple tables linked to each other by 'keys' i.e. unique identifiers. Non-relational is data that is difficult to organize into rows and columns. Relational databases are able to maintain a much higher degree of data integrety and make it much easier to filter the data to obtain only the data relevant to a specific query. However, this comes at the cost of higher cost and decreased scalability.
